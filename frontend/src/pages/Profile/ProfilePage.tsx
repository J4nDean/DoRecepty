  ...
  <div>
    ...
    <div className="relative">
      <input
        ...
        required
        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10"
      />
      <button
        type="button"
        onClick={toggle}
      ...
      <div className="max-w-2xl space-y-4 sm:space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {fields.map(({ label, value, mono }) => (
              <div key={label} className="min-w-0">
                ...
                <p className={`text-sm font-medium text-slate-800 break-all ${mono ? 'font-mono' : ''}`}>
                  {value ?? '—'}
                </p>
              </div>
            ))}
        ...
        <section className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <h3 className="text-sm font-bold text-slate-800">Zmiana hasła</h3>
          </div>
          ...
          {error && (
            <div className="flex items-center gap-2 text-red-700 bg-red-50 rounded-lg px-3 py-2.5 text-sm mb-4">
              <AlertCircle size={15} />
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold text-sm rounded-lg px-4 py-2.5 transition-colors mt-1"
            >
