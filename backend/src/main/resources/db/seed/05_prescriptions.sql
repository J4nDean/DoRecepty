...
FROM (VALUES
  ...
  ('PKG-KOW-005', '1005', -12,  18,  '9988776', '987654321', 'CZĘŚCIOWO_ZREALIZOWANA'),
  ('PKG-KOW-006', '1006', -5,   25,  '7766554', '123456789', 'CZĘŚCIOWO_ZREALIZOWANA'),
  ('PKG-KOW-007', '1007', -40,  -10, '7766554', '123456789', 'AKTYWNA'),
  ('PKG-KOW-008', '1008', -100, -70, '9988776', '987654321', 'ZREALIZOWANA')
...
INSERT INTO prescription_item (prescription_id, prescription_oid, oid_prefix, position_in_package, medication_id, quantity, dosage_instructions, realization_date_from, refund_level, status) VALUES
  ((SELECT id FROM prescription WHERE access_code='1001'), 'OID.1001.01', 'PL.P1.2026', 1, (SELECT id FROM medication WHERE name='Metformina Aurovitas'), 2, '2 x 1 tabl. do posiłku', NULL, '100%', 'ACTIVE'),
  ((SELECT id FROM prescription WHERE access_code='1002'), 'OID.1002.01', 'PL.P1.2026', 1, (SELECT id FROM medication WHERE name='Concor 10'), 1, '1 x 1 tabl. rano', NULL, 'ryczałt', 'ACTIVE'),
  ...
  ((SELECT id FROM prescription WHERE access_code='1005'), 'OID.1005.01', 'PL.P1.2026', 1, (SELECT id FROM medication WHERE name='Ventolin'), 1, 'W razie duszności: 1-2 dawki', NULL, 'ryczałt', 'REALIZED'),
  ...
  ((SELECT id FROM prescription WHERE access_code='1006'), 'OID.1006.01', 'PL.P1.2026', 1, (SELECT id FROM medication WHERE name='Amlozek'), 1, '1 x 1 tabl. rano', NULL, 'ryczałt', 'REALIZED'),
  ((SELECT id FROM prescription WHERE access_code='1006'), 'OID.1006.02', 'PL.P1.2026', 2, (SELECT id FROM medication WHERE name='Bisoprolol VP'), 1, '1 x 1 tabl. rano', NULL, 'ryczałt', 'ACTIVE'),
  ...
  ((SELECT id FROM prescription WHERE access_code='1008'), 'OID.1008.01', 'PL.P1.2026', 1, (SELECT id FROM medication WHERE name='Apap'), 1, '1 tabl. w razie bólu, max 4 x dziennie', NULL, NULL, 'REALIZED');
