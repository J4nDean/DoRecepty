...
FROM prescription p JOIN medication m ON m.name = 'Apap'
WHERE p.access_code = '8810';

INSERT INTO prescription_item (prescription_id, prescription_oid, oid_prefix, position_in_package, medication_id, quantity, dosage_instructions, status)
SELECT p.id, 'PL.CSIOZ.2024.90010000000002', 'PL.CSIOZ.2024', 2, m.id, 1, '1 x 1 tabl. po posiłku', 'REALIZED'
FROM prescription p JOIN medication m ON m.name = 'Ibuprom RR MAX'
WHERE p.access_code = '8810';

-- ── Recepta archiwalna 2: refluks (zrealizowana) ────────────────────────────
INSERT INTO prescription (package_key, access_code, issue_date, expiration_date, patient_id, doctor_npwz, clinic_regon, status)
...
FROM prescription p JOIN medication m ON m.name = 'Polprazol'
WHERE p.access_code = '8821';

-- ── Recepta archiwalna 3: cholesterol (archiwalna) ──────────────────────────
INSERT INTO prescription (package_key, access_code, issue_date, expiration_date, patient_id, doctor_npwz, clinic_regon, status)
...
FROM prescription p JOIN medication m ON m.name = 'Sortis 20'
WHERE p.access_code = '8832';

-- ── Recepta archiwalna 4: anulowana (zmiana terapii) ────────────────────────
INSERT INTO prescription (package_key, access_code, issue_date, expiration_date, patient_id, doctor_npwz, clinic_regon, status)
...
FROM prescription p JOIN medication m ON m.name = 'Norvasc'
WHERE p.access_code = '8843';

-- ── Recepta aktywna wygasająca za 7 dni (test powiadomienia) ─────────────────
INSERT INTO prescription (package_key, access_code, issue_date, expiration_date, patient_id, doctor_npwz, clinic_regon, status)
...
FROM prescription p JOIN medication m ON m.name = 'Metformina Aurovitas'
WHERE p.access_code = '9977';
